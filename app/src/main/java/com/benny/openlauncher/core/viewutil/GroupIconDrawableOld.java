...
public class GroupIconDrawableOld extends Drawable implements IconDrawer {
    ...
    public GroupIconDrawableOld(Context context, Item item, int iconSize) {
        int i;
    ...
    private void init(Drawable[] icons, float size) {
        this.icons = icons;
        this.iconSize = size;
        this.iconSizeDiv2 = Math.round(this.iconSize / Tool.DEFAULT_IMAGE_BACKOFF_MULT);
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.icons[0] != null) {
            drawIcon(canvas, this.icons[0], this.padding, this.padding, ((float) this.iconSizeDiv2) - this.padding, ((float) this.iconSizeDiv2) - this.padding, this.paintIcon);
        }
        if (this.icons[1] != null) {
            drawIcon(canvas, this.icons[1], ((float) this.iconSizeDiv2) + this.padding, this.padding, this.iconSize - this.padding, ((float) this.iconSizeDiv2) - this.padding, this.paintIcon);
        }
        if (this.icons[2] != null) {
            drawIcon(canvas, this.icons[2], this.padding, ((float) this.iconSizeDiv2) + this.padding, ((float) this.iconSizeDiv2) - this.padding, this.iconSize - this.padding, this.paintIcon);
        }
        if (this.icons[4] != null) {
            drawIcon(canvas, ContextCompat.getDrawable(this.context, R.drawable.ic_add_black_48dp), ((float) this.iconSizeDiv2) + this.padding, ((float) this.iconSizeDiv2) + this.padding, this.iconSize - this.padding, this.iconSize - this.padding, this.paintIcon);
        } else if (this.icons[3] != null) {
            drawIcon(canvas, this.icons[3], ((float) this.iconSizeDiv2) + this.padding, ((float) this.iconSizeDiv2) + this.padding, this.iconSize - this.padding, this.iconSize - this.padding, this.paintIcon);
        }
        canvas.restore();
        if (this.needAnimate) {
        ...
        try {
            float cx = AppItemView.cx_Notify;
            float cy = AppItemView.cy_Notify;
            if (cx != 0.0f || cy != 0.0f) {
                int numberNotify = 0;
                ...
                if (numberNotify > 0) {
                    ...
                    canvas.translate(cx - ((float) (rect.width() / 2)), ((float) (rect.height() / 2)) + cy);
