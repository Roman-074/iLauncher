...
public class SmoothViewPager extends ViewGroup {
    ...
    private static final Interpolator sInterpolator = new Interpolator() {
        public float getInterpolation(float t) {
            t -= Tool.DEFAULT_BACKOFF_MULT;
            return ((((t * t) * t) * t) * t) + Tool.DEFAULT_BACKOFF_MULT;
        }
    };
    ...
    private final Runnable mEndScrollRunnable = new Runnable() {
        public void run() {
            SmoothViewPager.this.setScrollState(0);
            SmoothViewPager.this.populate();
        }
    };
    ...
    public static class LayoutParams extends android.view.ViewGroup.LayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray a = context.obtainStyledAttributes(attrs, SmoothViewPager.LAYOUT_ATTRS);
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            if (event.getEventType() == 4096 && SmoothViewPager.this.mAdapter != null) {
                recordCompat.setItemCount(SmoothViewPager.this.mAdapter.getCount());
                recordCompat.setFromIndex(SmoothViewPager.this.mCurItem);
                recordCompat.setToIndex(SmoothViewPager.this.mCurItem);
            }
        }
        ...
        public boolean performAccessibilityAction(View host, int action, Bundle args) {
            ...
            switch (action) {
                case 4096:
                    ...
                    SmoothViewPager.this.setCurrentItem(SmoothViewPager.this.mCurItem + 1);
                    return true;
                case 8192:
                    if (!SmoothViewPager.this.canScrollHorizontally(-1)) {
                        return false;
        ...
        private boolean canScroll() {
            return SmoothViewPager.this.mAdapter != null && SmoothViewPager.this.mAdapter.getCount() > 1;
        }
    }

    interface OnAdapterChangeListener {
        void onAdapterChanged(SmoothPagerAdapter smoothPagerAdapter, SmoothPagerAdapter smoothPagerAdapter2);
    }
    ...
    private class PagerObserver extends DataSetObserver {
        ...
        public void onChanged() {
            SmoothViewPager.this.dataSetChanged();
        }
    ...
    void initViewPager() {
        ...
        this.mCloseEnough = (int) (Tool.DEFAULT_IMAGE_BACKOFF_MULT * density);
    ...
    public void setAdapter(SmoothPagerAdapter adapter) {
        ...
        SmoothPagerAdapter oldAdapter = this.mAdapter;
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(Tool.DEFAULT_BACKOFF_MULT, (Tool.DEFAULT_BACKOFF_MULT * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((Tool.DEFAULT_BACKOFF_MULT + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - Tool.DEFAULT_BACKOFF_MULT : AutoScrollHelper.NO_MAX;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - Tool.DEFAULT_BACKOFF_MULT;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
    ...
    private int determineTargetPage(int currentPage, float pageOffset, int velocity, int deltaX) {
        ...
        if (Math.abs(deltaX) <= this.mFlingDistance || Math.abs(velocity) <= this.mMinimumVelocity) {
            targetPage = (int) ((currentPage >= this.mCurItem ? 0.4f : 0.6f) + (((float) currentPage) + pageOffset));
        } else {
            targetPage = velocity > 0 ? currentPage : currentPage + 1;
        }
        return this.mItems.size() <= 0 ? targetPage : Math.max(((ItemInfo) this.mItems.get(0)).position, Math.min(targetPage, ((ItemInfo) this.mItems.get(this.mItems.size() - 1)).position));
    }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (currentFocused == this) {
            ...
            for (SmoothViewPager parent = (SmoothViewPager) currentFocused.getParent(); parent instanceof ViewGroup; parent = (SmoothViewPager) parent.getParent()) {
                if (parent == this) {
                    isChild = true;
                    break;
                }
